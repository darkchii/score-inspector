    ...
    useEffect(() => {
        if (props.data.processed.completion === undefined || props.data.processed.completion.cs === undefined) {
            (async () => {
                ...
                var __data = [];
                try {
                    __data[0] = parseFloat((await axios.get(`${(!process.env.NODE_ENV || process.env.NODE_ENV === 'development') ? config.OSU_TEST_API : config.OSU_API}beatmaps/count?cs_min=0&cs_max=1`, { headers: { "Access-Control-Allow-Origin": "*" } })).data);
                    __data[1] = parseFloat((await axios.get(`${(!process.env.NODE_ENV || process.env.NODE_ENV === 'development') ? config.OSU_TEST_API : config.OSU_API}beatmaps/count?cs_min=1&cs_max=2`, { headers: { "Access-Control-Allow-Origin": "*" } })).data);
                    __data[2] = parseFloat((await axios.get(`${(!process.env.NODE_ENV || process.env.NODE_ENV === 'development') ? config.OSU_TEST_API : config.OSU_API}beatmaps/count?cs_min=2&cs_max=3`, { headers: { "Access-Control-Allow-Origin": "*" } })).data);
                    __data[3] = parseFloat((await axios.get(`${(!process.env.NODE_ENV || process.env.NODE_ENV === 'development') ? config.OSU_TEST_API : config.OSU_API}beatmaps/count?cs_min=3&cs_max=4`, { headers: { "Access-Control-Allow-Origin": "*" } })).data);
                    __data[4] = parseFloat((await axios.get(`${(!process.env.NODE_ENV || process.env.NODE_ENV === 'development') ? config.OSU_TEST_API : config.OSU_API}beatmaps/count?cs_min=4&cs_max=5`, { headers: { "Access-Control-Allow-Origin": "*" } })).data);
                    __data[5] = parseFloat((await axios.get(`${(!process.env.NODE_ENV || process.env.NODE_ENV === 'development') ? config.OSU_TEST_API : config.OSU_API}beatmaps/count?cs_min=5&cs_max=6`, { headers: { "Access-Control-Allow-Origin": "*" } })).data);
