...
export function getPerformance2021July(data, debug = false) {
    const sr_model = '2019';
...
function getAimValue(data) {
    ...
    if (ar > 10.33) {
        approachRateFactor = ar - 10.33;
    } else if (ar < 8) {
        approachRateFactor = 0.025 * (8.0 - ar);
    }

    let approachRateTotalHitsFactor = 1.0 / (1.0 + Math.exp(-(0.007 * (amountTotalHits - 400))));
    let approachRateBonus = 1.0 + (0.03 + 0.37 * approachRateTotalHitsFactor) * approachRateFactor;

    if(data.score.enabled_mods & mods.HD){
        aim *= 1.0 + 0.04 * (12.0 - ar);
    }

    let flashlightBonus = 1.0;
    if(data.score.enabled_mods & mods.FL){
        flashlightBonus = 1.0 + 0.35 * Math.min(1.0, amountTotalHits / 200.0) +
            (amountTotalHits > 200 ? 0.3 * Math.min(1.0, (amountTotalHits - 200) / 300.0) +
                (amountTotalHits > 500 ? (amountTotalHits - 500) / 1200.0 : 0.0) : 0.0);
    }

    aim *= Math.max(flashlightBonus, approachRateBonus);
...
function getSpeedValue(data) {
    ...
    speed *= 1.0 + (0.03 + 0.37 * approachRateTotalHitsFactor) * approachRateFactor;
