...
export function getPerformance2014May(data, debug = false) {
    let sr_model = '2014may';
...
function getAccuracyValue(data) {
    let betterAccuracyPercentage = (data.circles > 0 ? 1 : 0) *
        Math.max(0, ((data.count300 - (data.totalhits - data.circles)) * 6 + data.count100 * 2 + data.count50) / (data.circles * 6));

    let acc = Math.pow(1.52163, data.modded_sr.modded_od) * Math.pow(betterAccuracyPercentage, 24) * 2.738;

    acc *= Math.min(Math.pow((data.circles / 1000.0), 0.3), 1.15);
