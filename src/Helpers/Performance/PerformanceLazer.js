...
export function getPerformanceLazer(data, debug = false) {
    const sr_model = 'live';
    ...
    data.modded_sr = score.beatmap.modded_sr[sr_model];
    ...
    if ((data.score.enabled_mods & mods.RX) !== 0) {
        let okMultiplier = Math.Max(0.0, data.modded_sr.modded_od > 0.0 ? 1 - Math.Pow(data.modded_sr.modded_od / 13.33, 1.8) : 1.0);
        let mehMultiplier = Math.Max(0.0, data.modded_sr.modded_od > 0.0 ? 1 - Math.Pow(data.modded_sr.modded_od / 13.33, 5) : 1.0);
...
function getAimValue(data) {
    ...
    aimValue *= 0.98 + ((data.modded_sr.modded_od * data.modded_sr.modded_od) / 2500);
    return aimValue;
}
...
function getSpeedValue(data) {
    ...
    if ((data.score.enabled_mods & mods.HD) !== 0) {
        speedValue *= (1.0 + 0.04 * (12 - data.modded_sr.modded_ar));
    }
